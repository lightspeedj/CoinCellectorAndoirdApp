package com.jackmok.gamedevelopment.game;
...
public class CoinMan extends ApplicationAdapter {
	...
	public void render () {
		...
		if (gameState == 0){
			...
			// user input for manipulating velocity
			if(Gdx.input.justTouched()){
				velocity = -10;
			}
			...
			//velocity calculation for jumping
			velocity += gravity;
			manY -= velocity;

			// man fall animation to the ground
			if(manY <= 0){
				manY = 0;
			}

			//initialisation of physical collision with coins
			manRectangle = new Rectangle(Gdx.graphics.getWidth()/2 - man[0].getWidth()/2, manY, man[manState].getWidth(), man[manState].getHeight());

			//physical collision with coins
